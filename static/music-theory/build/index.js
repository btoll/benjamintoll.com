...
_peteDom.dom.ready(() => {
  ...
  _peteDom.ajax.load({
    ...
    success: response => {
      ...
      const quizMap = {};
      const items = [];
      let quizName, quizText, links; // Construct the quiz menu from app.json.

      for (let i = 0, len = qs.length; i < len; i++) {
        let q = qs[i];
        quizName = q.name;
        quizText = quizName.replace(reReplaceUnderscore, ' ').replace(reUppercase, (a, $1) => $1.toUpperCase());
      ...
      links = _peteDom.element.gets('nav a');
      links.on('click', e => {
        ...
        e.preventDefault(); // Return early if already downloaded.

        if (cachedQuizzes[quizName]) {
          _peteDom.element.get(iframeId).toggleClass('hide');
        ...
        _peteDom.dom.create({
          ...
          success: response => {
            const doc = _peteDom.dom.getDom(iframeId).contentDocument;

            const tpl = _peteCore.core.create(_peteDom.template, {
              html: response
            });
...
},{"pete-core":3,"pete-dom":9}],2:[function(require,module,exports){
"use strict";
