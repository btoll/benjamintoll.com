  ...
  if (!Array.isArray(v)) {
    ...
    els.forEach(function (dom) {
      ...
      if (data.subscribe) {
        ...
        for (var _i = 0, _Object$keys = Object.keys(subscribe); _i < _Object$keys.length; _i++) {
...
var _default = dd;
exports["default"] = _default;
},{"pete-core":2,"pete-dom":9}],5:[function(require,module,exports){
"use strict";
...
var _default = ajax;
exports["default"] = _default;
},{"./util":12,"pete-core":2}],6:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2018 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
...
exports["default"] = _default;
},{"./element":8,"./pete":10,"pete-core":2}],7:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2018 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
...
var _default = dom;
exports["default"] = _default;
},{"./element":8,"pete-core":2}],8:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2018 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
...
exports["default"] = _default;
},{"./composite":6,"./dom":7,"./pete":10,"pete-core":2}],9:[function(require,module,exports){
"use strict";
...
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }
},{"./ajax":5,"./composite":6,"./dom":7,"./element":8,"./pete":10,"./template":11,"./util":12}],10:[function(require,module,exports){
"use strict";
...
var _default = pete;
exports["default"] = _default;
},{"./util":12,"pete-core":2}],11:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2018 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
...
exports["default"] = _default;
},{"./dom":7,"./element":8,"pete-core":2}],12:[function(require,module,exports){
"use strict";
...
var _default = util;
exports["default"] = _default;
},{"./pete":10}],13:[function(require,module,exports){
"use strict";

var _peteCore = require("pete-core");

var _peteDd = require("pete-dd");

var _peteDd2 = _interopRequireDefault(_peteDd);
...
const init = () => {
  ...
  const dragdrop = _peteCore.core.create(_peteDd2.default, {
    dragCls: 'notes'
  });

  dragdrop.initDD(_peteDom.element.get('notes'), {
    sort: true
  });
  dragdrop.initDD(_peteDom.element.gets('.dropZone'), {
    // dropProxy: false,
    subscribe: {
      beforenodedrop: function (e, context) {
        // Only drop if there isn't another child element in the target drop zone.
        if (_peteDom.element.gets(`.${context.dragCls}`, e.target).getCount()) {
          return false;
        }
      },
      afternodedrop: (e, context) => {
        const dragged = _peteDom.element.gets(`#dropZoneContainer .${context.dragCls}`, true);
        ...
        if ((draggedLen = dragged.length) === 4) {
          ...
          if (_peteDom.dom.getDom('currentChord').currentChord === arr.join('')) {
            alert('Correct!');
            reset();
            getChord();
          } else {
  ...
  setQuiz(); // Get first chord permutation.
...
const notes = []; // Holds the deep copy of either the notesObj object or both the notesObj and notesObjAdvanced objects.
...
const random = () => Math.round(Math.random()) - 0.5; // Collect all of the arrays from within the deepCopy object.
...
const getArrays = () => {
  ...
  const chords = [];
  let gotChords = false,
      tone; // A deep copy must be made every time in case the user selects a different skill level
  ...
  const deepCopy = (() => {
    ...
    return _peteCore.core.mixin(o, notesObjEnharmonic);
  })(); // Reset the chords and permutations array.


  chords.length = permutations.length = 0; // Get the permutations for the chordBuilder view.
...
const getChord = function () {
  ...
  _peteDom.dom.getDom('currentChord').innerHTML = '<span>' + permutations[n].chord + '</span>'; // We need to attach the array to an expando property since we need another way of comparing than
  ...
  _peteDom.dom.getDom('currentChord').currentChord = permutations[n].notes;
  n++;
};
...
const setQuiz = () => {
  for (let i = 0, len = notes.length; i < len; i++) {
    _peteDom.dom.create({
      ...
      parent: _peteDom.dom.getDom('notes')
    });
  }
};
...
const reset = function () {
  // Get all child nodes within the drop zone that have a 'sortOrder' property.
  const arr = Array.from(_peteDom.dom.getDom('notes').childNodes).concat(Array.from(_peteDom.element.gets('#dropZoneContainer .notes', true))).filter(v => // Should there be a better check?
  typeof v.sortOrder === 'number');
  const frag = document.createDocumentFragment();

  const dropZone = _peteDom.element.get('notes'); // Sort all nodes in this drop zone by their sort order property.


  arr.sort((a, b) => a.sortOrder - b.sortOrder); // Remove all the nodes...

  dropZone.remove(true); // ...and read them to the document fragment.
...
Promise.all([new Promise((resolve, reject) => _peteDom.ajax.load({
  ...
  data: 'json',
  success: data => (notesObjEnharmonic = data, resolve())
})), new Promise((resolve, reject) => _peteDom.ajax.load({
  url: 'build/chords/sevenths/basic.json',
  data: 'json',
  success: data => (notesObj = data, resolve())
})), new Promise((resolve, reject) => _peteDom.ajax.load({
  url: 'build/chords/sevenths/advanced.json',
  data: 'json',
  success: data => (notesObjAdvanced = data, resolve())
}))]).then(init);

_peteDom.dom.ready(function () {
  _peteDom.dom.create({
    tag: 'div',
    id: 'chordBuilder',
    items: [{
  ...
  _peteDom.element.get('.skipChord').on('click', skipChord); // If user agent is an iphone tweak the styles so everything fits in the screen.
...
},{"pete-core":2,"pete-dd":4,"pete-dom":9}]},{},[13]);
