...
const getChord = () => {
  ...
  _peteDom.dom.getDom('currentKeySignature').innerHTML = '<span>' + quizzes[n].join('</span><span>'); // We need to attach the array to an expando property since we need another way of comparing than
  ...
  _peteDom.dom.getDom('currentKeySignature').currentKeySignature = quizzes[n];
  n++;
};
...
_peteDom.dom.ready(() => {
  ...
  _peteDom.element.fly('keySignatures').on('click', e => {
    ...
    if (target.nodeName === 'SPAN') {
      ...
      if (notesObj[note] === _peteDom.dom.getDom('currentKeySignature').currentKeySignature) {
        alert('Correct');
        getChord();
      } else {
        alert('Incorrect');
