...
const makeDeepCopy = orig => {
  let o, v; // Arrays are handled differently than objects.

  if (orig instanceof Array) {
    o = [];

    for (let i = 0, len = orig.length; i < len; i++) {
      v = orig[i]; // Could be an array or an object.
    ...
    for (let prop of Object.keys(orig)) {
      v = orig[prop]; // Could be an array or an object.
...
const getChord = () => {
  ...
  _peteDom.element.gets('span').removeClass('selected');
  ...
  _peteDom.dom.getDom('currentChord').innerHTML = '<span>' + permutations[n].join('</span><span>'); // We need to attach the array to an expando property since we need another way of comparing than
  ...
  _peteDom.dom.getDom('currentChord').currentChord = permutations[n];
  n++;
}; // The types of chords that display (i.e., 'Major7', 'HalfDiminished7') depend on the properties of the deepCopy object.


const init = level => {
  skillLevel = level || 'advanced'; // Reset the counter.

  n = 0; // First lookup the skill level to see if the object needs to be created.
  ...
  setQuiz();
  _peteDom.dom.getDom('inversions').style.visibility = skillLevel !== 'beginner' ? 'visible' : 'hidden'; // Get first chord permutation.
...
const notes = [];
const inversions = ['RootPosition', 'FirstInversion', 'SecondInversion', 'ThirdInversion']; // Holds the deep copy of either the notesObj object or both the notesObj and notesObjAdvanced objects.

let deepCopy = {};
const permutations = [];
let skillLevel, notesObj, notesObjAdvanced;

const random = () => Math.round(Math.random()) - 0.5; // Instead of hardcoding the inversions, make them on the fly and bind them to the deepCopy object.
...
const getArrays = function () {
  ...
  let gotChords = false,
      tone,
      chord; // A deep copy must be made every time in case the user selects a different skill level
  ...
  deepCopy = skillLevel !== 'advanced' ? makeDeepCopy(notesObj) : function () {
    const o = makeDeepCopy(notesObj);

    for (let p in notesObj) {
      o[p] = _peteCore.core.mixin(o[p], notesObjAdvanced[p]);
    }
  ...
  for (let note of Object.keys(deepCopy)) {
    ...
    tone = deepCopy[note]; // If the tone object is false then skip it, i.e., c-flat and e-sharp.
    ...
    gotChords = true; // Sorting them frequently does a better job at randomizing them.
...
const setQuiz = () => {
  const setElements = (a, name) => {
    ...
    _peteDom.element.gets('#' + name + ' a').remove();
    ...
    for (let i = 0, len = a.length; i < len; i++) {
      _peteDom.dom.create({
        ...
        parent: _peteDom.element.gets('#' + name + ' div').elements[0]
      });
    }
  };
  ...
  if (!setQuiz.initiated) {
    for (let i = 0, len = notes.length; i < len; i++) {
      _peteDom.dom.create({
        ...
        // TODO: This is gnarly just to get a dom el.
        parent: _peteDom.element.gets('#notes div').elements[0]
      });
    }
...
Promise.all([new Promise((resolve, reject) => _peteDom.ajax.load({
  url: 'build/chords/sevenths/basic.json',
  data: 'json',
  success: (data, resp) => (notesObj = data, resolve())
})), new Promise((resolve, reject) => _peteDom.ajax.load({
  url: 'build/chords/sevenths/advanced.json',
  data: 'json',
  success: (data, resp) => (notesObjAdvanced = data, resolve())
}))]).then(() => init());
...
_peteDom.dom.ready(() => {
  ...
  _peteDom.element.fly('chordPuzzle').on('click', e => {
    ...
    if (target.nodeName === 'SPAN' && target.className !== 'blank') {
      ...
      _peteDom.element.gets('span', _peteDom.dom.getDom(target.className.trim())).removeClass('selected');

      _peteDom.element.fly(target).addClass('selected');

      numSelected = _peteDom.element.gets('#chordPuzzle span.selected').getCount(); // User selected one of each so see if they selected correctly.

      if (numSelected === 3 || numSelected === 2 && skillLevel === 'beginner') {
        note = _peteDom.element.get('#notes .selected').dom.note;
        chord = _peteDom.element.get('#chords .selected').value();
        inversion = skillLevel === 'beginner' ? inversion = 'RootPosition' : // Remove the space that was put in when the dom element was created, i.e., 'Second Inversion'.
        _peteDom.element.get('#inversions .selected').value().replace(/\s/, ''); // Remember root position is the only inversion that doesn't have its inversion as part of its name in deepCopy.

        if (deepCopy[note][chord + (inversion === 'RootPosition' ? '' : inversion)] === _peteDom.dom.getDom('currentChord').currentChord) {
          alert('Correct!');
          getChord();
        } else {
          alert('Incorrect!');
        ...
        _peteDom.element.gets('span').removeClass('selected'); // If beginner skill level is selected, make sure the 'Root Position' element is given the selected
        // class (to understand why see the logic w/in the handler bound to the 'chordPuzzle' element).


        if (_peteDom.dom.getDom('beginner').checked) {
          _peteDom.element.get('#inversions span').addClass('selected');
        }
      }
  ...
  _peteDom.element.get('.skipChord').on('click', skip);

  _peteDom.element.gets('input[type=radio]').on('click', e => {
    const value = e.target.value;

    _peteDom.element.get('#inversions span').removeClass('selected'); // If beginner skill level is selected, make sure the 'Root Position' element is given the selected
    // class (to understand why see the logic w/in the handler bound to the 'chordPuzzle' element).
  ...
  if (navigator.userAgent.indexOf('iPhone') !== -1 || navigator.userAgent.indexOf('Android') !== -1) {
    _peteDom.dom.getDom('chords').style.width = '450px';
    _peteDom.dom.getDom('inversions').style.width = '330px';
  }
});


},{"pete-core":2,"pete-dom":8}]},{},[12]);
