...
const fade = function (elem, red, green, blue) {
  ...
  const repeat = () => fade(elem, newred, newgreen, newblue);
...
_peteDom.dom.ready(() => {
  ...
  const cards = []; // An array that holds the 2 selected cards.

  const selectedCards = []; // An array that holds the cards that have been successfully matched.

  const matched = [];

  const cardTable = _peteDom.element.get('cardTable');
  ...
  const resetCards = () => {
    const cardOne = _peteDom.dom.getDom(selectedCards[0][1]);

    const cardTwo = _peteDom.dom.getDom(selectedCards[1][1]);
    ...
    selectedCards.length = 0; // Re-attach the event handler.

    cardTable.on('click', showValue);
  };

  const compareCards = () => {
    const scorer = _peteDom.dom.getDom('scorer'); // Remove the listener while comparing so the user doesn't get click-happy.


    cardTable.un('click', showValue);

    if (parseInt(selectedCards[0][0], 10) === parseInt(selectedCards[1][0], 10)) {
      const matches = _peteDom.dom.getDom('matches'); // Increment the 'matches' score.


      matches.innerHTML = parseInt(matches.innerHTML, 10) + 1;
      matched.push(selectedCards[0][1], selectedCards[1][1]); //resetCards();
      ...
      const chances = _peteDom.dom.getDom('chances');

      fade(scorer, 238, 0, 0);
      scorer.innerHTML = 'Too bad, you lost a chance!'; // There was no match so reset their backgrounds images after 1.2 seconds.
      ...
      if ((chances.innerHTML = parseInt(chances.innerHTML, 10) - 1) === 0) {
        if (confirm('You lost! Play again?')) {
          ...
          _peteDom.dom.getDom('cardTable').style.display = 'none';
        }
      }
    }
  };
  ...
  const showValue = e => {
    const target = e.target; // Since there's only one handler for the entire page we need to make sure the user actually clicked on a card and discard all other events.

    if (reCard.test(target.id)) {
      target.firstChild.style.display = 'block';
      selectedCards.push([_peteDom.element.gets('span', target).getFirst().textContent, target.id]);
  ...
  const initGame = () => {
    const seed = (() => {
      let arr = []; // Make a duplicated 12 element array, non-inclusive.

      return (arr = _peteDom.util.range(`0...${totalCards / 2}`)).concat(arr);
    })();

    seed.sort(() => Math.round(Math.random()) - 0.5);
    cards.length = 0;
    _peteDom.dom.getDom('cardTable').innerHTML = '';
    _peteDom.dom.getDom('chances').innerHTML = chances;
    _peteDom.dom.getDom('matches').innerHTML = 0; // Each div MUST have a unique id, that's how this works :)

    for (let i = totalCards; i--;) {
      cards.push(_peteDom.dom.create({
        tag: 'div',
        attr: {
          id: 'card' + i
        },
    ...
    _peteDom.dom.getDom('cardTable').appendChild(frag); // Event delegation.
...
},{"pete-dom":9}],2:[function(require,module,exports){
"use strict";
...
},{"./core":2,"./observer":4}],4:[function(require,module,exports){
"use strict";
...
var _default = ajax;
exports["default"] = _default;
},{"./util":12,"pete-core":3}],6:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2020 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
...
exports["default"] = _default;
},{"./element":8,"./pete":10,"pete-core":3}],7:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2020 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
...
var _default = dom;
exports["default"] = _default;
},{"./element":8,"pete-core":3}],8:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2020 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
    ...
    closest: function closest(elem) {
      ...
      while (parent && parent.nodeName) {
        if (parent.nodeName.toLocaleLowerCase() === elem) {
          ...
          parent.parentNode, _readOnlyError("parent");
      ...
      type.forEach(function (type) {
        ...
        if (!o[type]) {
          ...
          if (!Array.isArray(o[type])) {
            Array.from(o), _readOnlyError("arr");
            arr.push(fn);
...
exports["default"] = _default;
},{"./composite":6,"./dom":7,"./pete":10,"pete-core":3}],9:[function(require,module,exports){
"use strict";
...
var isEdge = !isOpera && ua.indexOf('edge') > -1;
var isIE = !isOpera && ua.indexOf('msie') > -1;
var isIE6 = !isOpera && ua.indexOf('msie 6') > -1;
var isIE7 = !isOpera && ua.indexOf('msie 7') > -1;
var isIE8 = !isOpera && ua.indexOf('msie 8') > -1; //https://en.wikipedia.org/wiki/Internet_Explorer_9#User_agent_string

var isIE9 = !isOpera && ua.indexOf('trident/5.0') > -1; //https://en.wikipedia.org/wiki/Internet_Explorer_10#User_agent_string

var isIE10 = !isOpera && ua.indexOf('trident/6.0') > -1;
var isIE11 = !isOpera && (ua.indexOf('trident/7.0') > -1 || ua.indexOf('trident/8.0') > -1);
...
var _default = pete;
exports["default"] = _default;
},{"./util":12,"pete-core":3}],11:[function(require,module,exports){
/**
 * Copyright (c) 2009 - 2020 Benjamin Toll (benjamintoll.com)
 *
 * This file is part of pete-dom.
...
exports["default"] = _default;
},{"./dom":7,"./element":8,"pete-core":3}],12:[function(require,module,exports){
"use strict";
